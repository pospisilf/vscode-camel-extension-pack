...
import { DEMO_FILE, QUARKUS_ATTACH_DEBUGGER, QUARKUS_CREATE_COMMAND, QUARKUS_DIR, RESOURCES_DIR, SPRINGBOOT_ATTACH_DEBUGGER, SPRINGBOOT_CREATE_COMMAND, SPRINGBOOT_DIR, SPRINGBOOT_PROJECT_FOLDER } from '../variables';
...
describe('Remote debug on OpenShift', function () {
    ...
    describe.only('Spring Boot project', function () {
    ...
    async function prepareEnvironment(directory: string, directoryPath: string): Promise<void> {
        ...
        console.log('----------- resources dir -----------')
        readdirSync(RESOURCES_DIR).forEach(file => {
            console.log(file);
        });
        driver = VSBrowser.instance.driver
